  ...
  async (accounts) => {
    ...
    context("Batch liquidations", () => {
      ...
      it("Stability Pool profit matches", async () => {
        ...
        const expectedGainInZKUSD = expectedCollateralLiquidatedA
          .mul(price)
          .div(mv._1e18BN)
          .sub(A_totalDebt);
        const realGainInZKUSD = spEthAfter
          .sub(spEthBefore)
          .mul(price)
          .div(mv._1e18BN)
        ...
        assert.equal(
          ...
          "Stability Pool ZKUSD doesn’t match"
        );
        assert.equal(
          realGainInZKUSD.toString(),
          expectedGainInZKUSD.toString(),
