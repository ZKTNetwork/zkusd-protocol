...
const ZKUSDTokenTester = artifacts.require("./ZKUSDTokenTester");
...
/* NOTE: Some of the borrowing tests do not test for specific ZKUSD fee values. They only test that the
 * fees are non-zero when they should occur, and that they decay over time.
 *
 * Specific ZKUSD fee values will depend on the final fee schedule used, and the final choice for
  ...
  const testCorpus = ({ withProxy = false }) => {
    beforeEach(async () => {
      ...
      ZKUSD_GAS_COMPENSATION =
        await borrowerOperations.ZKUSD_GAS_COMPENSATION();
    ...
    //      const alicePendingZKUSDDebtReward = await troveManager.getPendingZKUSDDebtReward(alice)
    //      const bobPendingZKUSDDebtReward = await troveManager.getPendingZKUSDDebtReward(bob)
    //      for (reward of [alicePendingETHReward, bobPendingETHReward, alicePendingZKUSDDebtReward, bobPendingZKUSDDebtReward]) {
    ...
    //      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePendingZKUSDDebtReward)))
    //      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)))
    //      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingZKUSDDebtReward)))
    //
    ...
    //      assert.isAtMost(th.getDifference(alice_ZKUSDDebtRewardSnapshot_After, L_ZKUSDDebt), 100)
    //      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After, L_ETH), 100)
    //      assert.isAtMost(th.getDifference(bob_ZKUSDDebtRewardSnapshot_After, L_ZKUSDDebt), 100)
    //  })
    //
    ...
    //      const pendingDebtReward_B = await troveManager.getPendingZKUSDDebtReward(bob)
    ...
    //  // --- withdrawZKUSD() ---
    //
    //  it("withdrawZKUSD(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
    //      // alice creates a Trove and adds first collateral
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: alice}})
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: bob}})
    //
    ...
    //      assert.isTrue((await troveManager.getCurrentICR(alice, price)).lt(toBN(dec(110, 16))))
    //
    //      const ZKUSDwithdrawal = 1  // withdraw 1 wei ZKUSD
    //
    //      await assertRevert(borrowerOperations.withdrawZKUSD(th._100pct, ZKUSDwithdrawal, alice, alice, {from: alice}),
    //          "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    //  })
    //
    //  it("withdrawZKUSD(): decays a non-zero base rate", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //
    //      await openTrove({extraZKUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: E}})
    //
    //      const A_ZKUSDBal = await zkusdToken.balanceOf(A)
    //
    ...
    //
    //      // D withdraws ZKUSD
    //      await borrowerOperations.withdrawZKUSD(th._100pct, dec(1, 18), A, A, {from: D})
    //
    ...
    //
    //      // E withdraws ZKUSD
    //      await borrowerOperations.withdrawZKUSD(th._100pct, dec(1, 18), A, A, {from: E})
    //
    //      const baseRate_3 = await troveManager.baseRate()
    //      assert.isTrue(baseRate_3.lt(baseRate_2))
    //  })
    //
    //  it("withdrawZKUSD(): reverts if max fee > 100%", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    //
    //      await assertRevert(borrowerOperations.withdrawZKUSD(dec(2, 18), dec(1, 18), A, A, {from: A}), "Max fee percentage must be between 0.5% and 100%")
    //      await assertRevert(borrowerOperations.withdrawZKUSD('1000000000000000001', dec(1, 18), A, A, {from: A}), "Max fee percentage must be between 0.5% and 100%")
    //  })
    //
    //  it("withdrawZKUSD(): reverts if max fee < 0.5% in Normal mode", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    //
    //      await assertRevert(borrowerOperations.withdrawZKUSD(0, dec(1, 18), A, A, {from: A}), "Max fee percentage must be between 0.5% and 100%")
    //      await assertRevert(borrowerOperations.withdrawZKUSD(1, dec(1, 18), A, A, {from: A}), "Max fee percentage must be between 0.5% and 100%")
    //      await assertRevert(borrowerOperations.withdrawZKUSD('4999999999999999', dec(1, 18), A, A, {from: A}), "Max fee percentage must be between 0.5% and 100%")
    //  })
    //
    //  it("withdrawZKUSD(): reverts if fee exceeds max fee percentage", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(180, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: E}})
    //
    ...
    //
    //
    //      // ZKUSDFee:                  15000000558793542
    //      // absolute _fee:            15000000558793542
    //      // actual feePercentage:      5000000186264514
    //      // user's _maxFeePercentage: 49999999999999999
    //
    //      const lessThan5pct = '49999999999999999'
    //      await assertRevert(borrowerOperations.withdrawZKUSD(lessThan5pct, dec(3, 18), A, A, {from: A}), "Fee exceeded provided maximum")
    //
    //      baseRate = await troveManager.baseRate() // expect 5% base rate
    //      assert.equal(baseRate, dec(5, 16))
    //      // Attempt with maxFee 1%
    //      await assertRevert(borrowerOperations.withdrawZKUSD(dec(1, 16), dec(1, 18), A, A, {from: B}), "Fee exceeded provided maximum")
    //
    //      baseRate = await troveManager.baseRate()  // expect 5% base rate
    //      assert.equal(baseRate, dec(5, 16))
    //      // Attempt with maxFee 3.754%
    //      await assertRevert(borrowerOperations.withdrawZKUSD(dec(3754, 13), dec(1, 18), A, A, {from: C}), "Fee exceeded provided maximum")
    //
    //      baseRate = await troveManager.baseRate()  // expect 5% base rate
    //      assert.equal(baseRate, dec(5, 16))
    //      // Attempt with maxFee 0.5%%
    //      await assertRevert(borrowerOperations.withdrawZKUSD(dec(5, 15), dec(1, 18), A, A, {from: D}), "Fee exceeded provided maximum")
    //  })
    //
    //  it("withdrawZKUSD(): succeeds when fee is less than max fee percentage", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    ...
    //      const tx1 = await borrowerOperations.withdrawZKUSD(moreThan5pct, dec(1, 18), A, A, {from: A})
    ...
    //
    //      // Attempt with maxFee = 5%
    //      const tx2 = await borrowerOperations.withdrawZKUSD(dec(5, 16), dec(1, 18), A, A, {from: B})
    ...
    //      const tx3 = await borrowerOperations.withdrawZKUSD(dec(1, 17), dec(1, 18), A, A, {from: C})
    ...
    //      const tx4 = await borrowerOperations.withdrawZKUSD(dec(37659, 13), dec(1, 18), A, A, {from: D})
    //      assert.isTrue(tx4.receipt.status)
    //
    //      // Attempt with maxFee 100%
    //      const tx5 = await borrowerOperations.withdrawZKUSD(dec(1, 18), dec(1, 18), A, A, {from: E})
    //      assert.isTrue(tx5.receipt.status)
    //  })
    //
    //  it("withdrawZKUSD(): doesn't change base rate if it is already zero", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //
    //      await openTrove({extraZKUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: E}})
    //
    ...
    //
    //      // D withdraws ZKUSD
    //      await borrowerOperations.withdrawZKUSD(th._100pct, dec(37, 18), A, A, {from: D})
    //
    ...
    //
    //      // E opens trove
    //      await borrowerOperations.withdrawZKUSD(th._100pct, dec(12, 18), A, A, {from: E})
    //
    //      const baseRate_3 = await troveManager.baseRate()
    //      assert.equal(baseRate_3, '0')
    //  })
    //
    //  it("withdrawZKUSD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //
    //      await openTrove({extraZKUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    ...
    //      await borrowerOperations.withdrawZKUSD(th._100pct, dec(1, 18), C, C, {from: C})
    //
    ...
    //
    //
    //  it("withdrawZKUSD(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    ...
    //  })
    //
    //  it("withdrawZKUSD(): borrowing at non-zero base rate sends ZKUSD fee to ZKT staking contract", async () => {
    ...
    //
    //      // D withdraws ZKUSD
    //      await borrowerOperations.withdrawZKUSD(th._100pct, dec(37, 18), C, C, {from: D})
    //
    ...
    //      it("withdrawZKUSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
    //          // time fast-forwards 1 year, and multisig stakes 1 ZKT
    //          await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    //          await zkToken.approve(zktStaking.address, dec(1, 18), {from: multisig})
    //          await zktStaking.stake(dec(1, 18), {from: multisig})
    ...
    //          const withdrawalTx = await borrowerOperations.withdrawZKUSD(th._100pct, toBN(dec(37, 18)), D, D, {from: D})
    //
    //          const emittedFee = toBN(th.getZKUSDFeeFromZKUSDBorrowingEvent(withdrawalTx))
    ...
    //  }
    //
    //  it("withdrawZKUSD(): Borrowing at non-zero base rate increases the ZKT staking contract ZKUSD fees-per-unit-staked", async () => {
    //      // time fast-forwards 1 year, and multisig stakes 1 ZKT
    //      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    //      await zkToken.approve(zktStaking.address, dec(1, 18), {from: multisig})
    //      await zktStaking.stake(dec(1, 18), {from: multisig})
    ...
    //
    //      // D withdraws ZKUSD
    //      await borrowerOperations.withdrawZKUSD(th._100pct, toBN(dec(37, 18)), D, D, {from: D})
    //
    ...
    //  })
    //
    //  it("withdrawZKUSD(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
    //      // time fast-forwards 1 year, and multisig stakes 1 ZKT
    //      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    //      await zkToken.approve(zktStaking.address, dec(1, 18), {from: multisig})
    //      await zktStaking.stake(dec(1, 18), {from: multisig})
    ...
    //      th.fastForwardTime(7200, web3.currentProvider)
    //
    //      const D_ZKUSDBalanceBefore = await zkusdToken.balanceOf(D)
    //
    //      // D withdraws ZKUSD
    //      const D_ZKUSDRequest = toBN(dec(37, 18))
    //      await borrowerOperations.withdrawZKUSD(th._100pct, D_ZKUSDRequest, D, D, {from: D})
    //
    //      // Check ZKT staking ZKUSD balance has increased
    //      const zktStaking_ZKUSDBalance_After = await zkusdToken.balanceOf(zktStaking.address)
    //      assert.isTrue(zktStaking_ZKUSDBalance_After.gt(zktStaking_ZKUSDBalance_Before))
    //
    //      // Check D's ZKUSD balance now equals their initial balance plus request ZKUSD
    //      const D_ZKUSDBalanceAfter = await zkusdToken.balanceOf(D)
    //      assert.isTrue(D_ZKUSDBalanceAfter.eq(D_ZKUSDBalanceBefore.add(D_ZKUSDRequest)))
    //  })
    //
    //  it("withdrawZKUSD(): Borrowing at zero base rate changes ZKUSD fees-per-unit-staked", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    ...
    //      assert.equal(baseRate_1, '0')
    //
    //      // A artificially receives ZKT, then stakes it
    ...
    //
    //      // D withdraws ZKUSD
    //      await borrowerOperations.withdrawZKUSD(th._100pct, dec(37, 18), D, D, {from: D})
    //
    //      // Check ZKT ZKUSD balance after > 0
    //      const F_ZKUSD_After = await zktStaking.F_ZKUSD()
    //      assert.isTrue(F_ZKUSD_After.gt('0'))
    //  })
    //
    //  it("withdrawZKUSD(): Borrowing at zero base rate sends debt request to user", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    ...
    //      const D_ZKUSDBalanceAfter = await zkusdToken.balanceOf(D)
    //
    //      // Check D's trove debt == D's ZKUSD balance + liquidation reserve
    //      assert.isTrue(D_ZKUSDBalanceAfter.eq(D_ZKUSDBalanceBefore.add(D_ZKUSDRequest)))
    //  })
    //
    //  it("withdrawZKUSD(): reverts when calling address does not have active trove", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: alice}})
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: bob}})
    //
    //      // Bob successfully withdraws ZKUSD
    //      const txBob = await borrowerOperations.withdrawZKUSD(th._100pct, dec(100, 18), bob, bob, {from: bob})
    //      assert.isTrue(txBob.receipt.status)
    //
    //      // Carol with no active trove attempts to withdraw ZKUSD
    //      try {
    //          const txCarol = await borrowerOperations.withdrawZKUSD(th._100pct, dec(100, 18), carol, carol, {from: carol})
    //          assert.isFalse(txCarol.receipt.status)
    //      } catch (err) {
    //          assert.include(err.message, "revert")
    //      }
    //  })
    //
    //  it("withdrawZKUSD(): reverts when requested withdrawal amount is zero ZKUSD", async () => {
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: alice}})
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: bob}})
    //
    //      // Bob successfully withdraws 1e-18 ZKUSD
    //      const txBob = await borrowerOperations.withdrawZKUSD(th._100pct, 1, bob, bob, {from: bob})
    //      assert.isTrue(txBob.receipt.status)
    //
    //      // Alice attempts to withdraw 0 ZKUSD
    //      try {
    //          const txAlice = await borrowerOperations.withdrawZKUSD(th._100pct, 0, alice, alice, {from: alice})
    ...
    //  })
    //
    //  it("withdrawZKUSD(): reverts when system is in Recovery Mode", async () => {
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: alice}})
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: bob}})
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: carol}})
    //
    ...
    //
    //      // Withdrawal possible when recoveryMode == false
    //      const txAlice = await borrowerOperations.withdrawZKUSD(th._100pct, dec(100, 18), alice, alice, {from: alice})
    //      assert.isTrue(txAlice.receipt.status)
    //
    ...
    //      assert.isTrue(await th.checkRecoveryMode(contracts))
    //
    //      //Check ZKUSD withdrawal impossible when recoveryMode == true
    //      try {
    //          const txBob = await borrowerOperations.withdrawZKUSD(th._100pct, 1, bob, bob, {from: bob})
    //          assert.isFalse(txBob.receipt.status)
    //      } catch (err) {
    ...
    //      try {
    //          const txBob = await borrowerOperations.withdrawZKUSD(th._100pct, dec(1, 18), bob, bob, {from: bob})
    //          assert.isFalse(txBob.receipt.status)
    //      } catch (err) {
    //          assert.include(err.message, "revert")
    //      }
    ...
    //
    //      try {
    //          const txData = await borrowerOperations.withdrawZKUSD(th._100pct, '200', alice, alice, {from: alice})
    //          assert.isFalse(txData.receipt.status)
    //      } catch (err) {
    //          assert.include(err.message, 'revert')
    //      }
    ...
    //
    //      await borrowerOperations.withdrawZKUSD(th._100pct, await getNetBorrowingAmount(100), alice, alice, {from: alice})
    //
    //      // check after
    //      const aliceDebtAfter = await getTroveEntireDebt(alice)
    ...
    it("withdrawZKUSD(): increases ZKUSD debt in ActivePool by correct amount", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice);
      console.log(aliceDebtBefore.toString());
      assert.isTrue(aliceDebtBefore.gt(toBN(0)));

      // check before
      const activePool_ZKUSD_Before = await activePool.getZKUSDDebt();
      console.log(activePool_ZKUSD_Before.toString());
      assert.isTrue(activePool_ZKUSD_Before.eq(aliceDebtBefore));

      console.log((await getNetBorrowingAmount(dec(10000, 18))).toString());
      ...
      th.assertIsApproximatelyEqual(
        ...
        activePool_ZKUSD_Before.add(toBN(dec(10000, 18)))
      );
    });
    ...
    //
    //      await assertRevert(borrowerOperations.repayZKUSD(ZKUSDRepayment, alice, alice, {from: alice}),
    //          "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    //  })
    //
    ...
    //      })
    //
    //      const repayTxA = await borrowerOperations.repayZKUSD(1, A, A, {from: A})
    //      assert.isTrue(repayTxA.receipt.status)
    //
    //      await borrowerOperations.openTrove(th._100pct, dec(20, 25), B, B, {from: B, value: dec(100, 30)})
    //
    //      const repayTxB = await borrowerOperations.repayZKUSD(dec(19, 25), B, B, {from: B})
    ...
    //      })
    //
    //      const repayTxAPromise = borrowerOperations.repayZKUSD(2, A, A, {from: A})
    ...
    //      await openTrove({extraZKUSDAmount: repayAmount, ICR: toBN(dec(150, 16)), extraParams: {from: bob}})
    //
    ...
    //      const txBob = await borrowerOperations.repayZKUSD(dec(10, 18), bob, bob, {from: bob})
    //      assert.isTrue(txBob.receipt.status)
    //
    //      // Carol with no active trove attempts to repayZKUSD
    //      try {
    //          const txCarol = await borrowerOperations.repayZKUSD(dec(10, 18), carol, carol, {from: carol})
    //          assert.isFalse(txCarol.receipt.status)
    //      } catch (err) {
    //          assert.include(err.message, "revert")
    //      }
    ...
    //      try {
    //          const txAlice = await borrowerOperations.repayZKUSD(aliceDebt.add(toBN(dec(1, 18))), alice, alice, {from: alice})
    //          assert.isFalse(txAlice.receipt.status)
    //      } catch (err) {
    //          assert.include(err.message, "revert")
    //      }
    ...
    //      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
    //
    //      await borrowerOperations.repayZKUSD(aliceDebtBefore.div(toBN(10)), alice, alice, {from: alice})  // Repays 1/10 her debt
    //
    //      const aliceDebtAfter = await getTroveEntireDebt(alice)
    ...
    //      // check after
    //      const activePool_ZKUSD_After = await activePool.getZKUSDDebt()
    //      th.assertIsApproximatelyEqual(activePool_ZKUSD_After, activePool_ZKUSD_Before.sub(aliceDebtBefore.div(toBN(10))))
    //  })
    //
    ...
    //      // check after
    //      const alice_ZKUSDTokenBalance_After = await zkusdToken.balanceOf(alice)
    //      th.assertIsApproximatelyEqual(alice_ZKUSDTokenBalance_After, alice_ZKUSDTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10))))
    //  })
    //
    ...
    //      assert.isTrue(await th.checkRecoveryMode(contracts))
    //
    //      const tx = await borrowerOperations.repayZKUSD(aliceDebtBefore.div(toBN(10)), alice, alice, {from: alice})
    ...
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: alice}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    ...
    //      await zkusdToken.transfer(C, bobBalBefore.sub((toBN(dec(5, 18)))), {from: B})
    //
    ...
    //      const repayZKUSDPromise_B = borrowerOperations.repayZKUSD(toBN(dec(6, 18)), B, B, {from: B})
    //
    //      await assertRevert(repayZKUSDPromise_B, "Caller doesnt have enough ZKUSD to make repayment")
    //  })
    //
    //  // --- adjustTrove() ---
    //
    ...
    //      const collTopUp = 1
    //
    //      await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, ZKUSDRepayment, false, alice, alice, {
    //              from: alice,
    //              value: collTopUp
    //          }),
    //          "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    ...
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //
    ...
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: E}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    //
    //      // Check baseRate is zero
    //      const baseRate_1 = await troveManager.baseRate()
    ...
    //  })
    //
    //  it("adjustTrove(): borrowing at non-zero base rate sends ZKUSD fee to ZKT staking contract", async () => {
    //      // time fast-forwards 1 year, and multisig stakes 1 ZKT
    //      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    //      await zkToken.approve(zktStaking.address, dec(1, 18), {from: multisig})
    //      await zktStaking.stake(dec(1, 18), {from: multisig})
    ...
    //
    //          const emittedFee = toBN(th.getZKUSDFeeFromZKUSDBorrowingEvent(adjustmentTx))
    //          assert.isTrue(emittedFee.gt(toBN('0')))
    //
    ...
    //  }
    //
    //  it("adjustTrove(): Borrowing at non-zero base rate increases the ZKT staking contract ZKUSD fees-per-unit-staked", async () => {
    //      // time fast-forwards 1 year, and multisig stakes 1 ZKT
    //      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    //      await zkToken.approve(zktStaking.address, dec(1, 18), {from: multisig})
    //      await zktStaking.stake(dec(1, 18), {from: multisig})
    ...
    //
    //      // D adjusts trove
    //      const ZKUSDRequest_D = toBN(dec(40, 18))
    //      await borrowerOperations.adjustTrove(th._100pct, 0, ZKUSDRequest_D, true, D, D, {from: D})
    //
    //      // Check ZKT staking ZKUSD balance has increased
    //      const zktStaking_ZKUSDBalance_After = await zkusdToken.balanceOf(zktStaking.address)
    //      assert.isTrue(zktStaking_ZKUSDBalance_After.gt(zktStaking_ZKUSDBalance_Before))
    //
    //      // Check D's ZKUSD balance has increased by their requested ZKUSD
    //      const D_ZKUSDBalanceAfter = await zkusdToken.balanceOf(D)
    //      assert.isTrue(D_ZKUSDBalanceAfter.eq(D_ZKUSDBalanceBefore.add(ZKUSDRequest_D)))
    //  })
    //
    //  it("adjustTrove(): Borrowing at zero base rate changes ZKUSD balance of ZKT staking contract", async () => {
    //      await openTrove({ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: C}})
    ...
    //      th.fastForwardTime(7200, web3.currentProvider)
    //
    //      // Check staking ZKUSD balance before > 0
    //      const zktStaking_ZKUSDBalance_Before = await zkusdToken.balanceOf(zktStaking.address)
    //      assert.isTrue(zktStaking_ZKUSDBalance_Before.gt(toBN('0')))
    //
    //      // D adjusts trove
    //      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, {from: D})
    //
    //      // Check staking ZKUSD balance after > staking balance before
    //      const zktStaking_ZKUSDBalance_After = await zkusdToken.balanceOf(zktStaking.address)
    //      assert.isTrue(zktStaking_ZKUSDBalance_After.gt(zktStaking_ZKUSDBalance_Before))
    //  })
    //
    //  it("adjustTrove(): Borrowing at zero base rate changes ZKT staking contract ZKUSD fees-per-unit-staked", async () => {
    //      await openTrove({
    //          extraZKUSDAmount: toBN(dec(20000, 18)),
    //          ICR: toBN(dec(2, 18)),
    //          extraParams: {from: whale, value: toBN(dec(100, 'ether'))}
    //      })
    //      await openTrove({extraZKUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: A}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: B}})
    ...
    //      // Check staking ZKUSD balance before == 0
    //      const F_ZKUSD_Before = await zktStaking.F_ZKUSD()
    //      assert.isTrue(F_ZKUSD_Before.eq(toBN('0')))
    //
    //      // D adjusts trove
    //      await borrowerOperations.adjustTrove(th._100pct, 0, dec(37, 18), true, D, D, {from: D})
    //
    //      // Check staking ZKUSD balance increases
    //      const F_ZKUSD_After = await zktStaking.F_ZKUSD()
    //      assert.isTrue(F_ZKUSD_After.gt(F_ZKUSD_Before))
    //  })
    //
    //  it("adjustTrove(): Borrowing at zero base rate sends total requested ZKUSD to the user", async () => {
    //      await openTrove({
    //          extraZKUSDAmount: toBN(dec(20000, 18)),
    //          ICR: toBN(dec(2, 18)),
    //          extraParams: {from: whale, value: toBN(dec(100, 'ether'))}
    ...
    //      await openTrove({extraZKUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: D}})
    //
    //      const D_ZKUSDBalBefore = await zkusdToken.balanceOf(D)
    //      // Check baseRate is zero
    //      const baseRate_1 = await troveManager.baseRate()
    //      assert.equal(baseRate_1, '0')
    //
    ...
    //      th.fastForwardTime(7200, web3.currentProvider)
    //
    //      const DUSDBalanceBefore = await zkusdToken.balanceOf(D)
    //
    //      // D adjusts trove
    //      const ZKUSDRequest_D = toBN(dec(40, 18))
    //      await borrowerOperations.adjustTrove(th._100pct, 0, ZKUSDRequest_D, true, D, D, {from: D})
    //
    //      // Check D's ZKUSD balance increased by their requested ZKUSD
    //      const ZKUSDBalanceAfter = await zkusdToken.balanceOf(D)
    //      assert.isTrue(ZKUSDBalanceAfter.eq(D_ZKUSDBalBefore.add(ZKUSDRequest_D)))
    //  })
    //
    //  it("adjustTrove(): reverts when calling address has no active trove", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: alice}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: bob}})
    //
    ...
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(3, 18)), extraParams: {from: alice}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: bob}})
    //      const CCR = await troveManager.CCR()
    //
    //      assert.isFalse(await th.checkRecoveryMode(contracts))
    ...
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: alice}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(200000, 18)), ICR: toBN(dec(2, 18)), extraParams: {from: bob}})
    //
    //      assert.isFalse(await th.checkRecoveryMode(contracts))
    //
    //      await priceFeed.setPrice(dec(120, 18)) // trigger drop in ETH price
    ...
    //
    //      // B stakes ZKT
    //      await zkToken.unprotectedMint(bob, dec(100, 18))
    //      await zktStaking.stake(dec(100, 18), {from: bob})
    //
    //      const zktStakingZKUSDBalanceBefore = await zkusdToken.balanceOf(zktStaking.address)
    //      assert.isTrue(zktStakingZKUSDBalanceBefore.gt(toBN('0')))
    //
    //      const txAlice = await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, {
    //          from: alice,
    //          value: dec(100, 'ether')
    ...
    //
    //      // Check emitted fee = 0
    //      const emittedFee = toBN(await th.getEventArgByName(txAlice, 'ZKUSDBorrowingFeePaid', '_ZKUSDFee'))
    //      assert.isTrue(emittedFee.eq(toBN('0')))
    //
    //      assert.isTrue(await th.checkRecoveryMode(contracts))
    //
    //      // Check no fee was sent to staking contract
    //      const zktStakingZKUSDBalanceAfter = await zkusdToken.balanceOf(zktStaking.address)
    //      assert.equal(zktStakingZKUSDBalanceAfter.toString(), zktStakingZKUSDBalanceBefore.toString())
    //  })
    //
    ...
    //  })
    //
    //  it("adjustTrove(): reverts when ZKUSD repaid is > debt of the trove", async () => {
    //      await openTrove({ICR: toBN(dec(2, 18)), extraParams: {from: alice}})
    ...
    //
    //      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, 'ZKUSDBorrowingFeePaid', 1))
    //      assert.isTrue(bobFee.gt(toBN('0')))
    //
    //      // Alice transfers ZKUSD to bob to compensate borrowing fees
    //      await zkusdToken.transfer(bob, bobFee, {from: alice})
    //
    //      const remainingDebt = (await troveManager.getTroveDebt(bob)).sub(ZKUSD_GAS_COMPENSATION)
    //
    ...
    //      await priceFeed.setPrice(dec(100, 18))
    //
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: {from: alice}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: {from: bob}})
    //
    //      // Bob attempts to increase debt by 100 ZKUSD and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    //
    //      // Alice adjusts trove. No coll change, and a debt increase (+50ZKUSD)
    //      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, {
    //          from: alice,
    //          value: 0
    //      })
    ...
    //
    //      const aliceDebtBefore = await getTroveEntireDebt(alice)
    //      const activePoolDebtBefore = await activePool.getZKUSDDebt()
    //
    //      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
    ...
    //
    //      const aliceDebtAfter = await getTroveEntireDebt(alice)
    //      const activePoolDebtAfter = await activePool.getZKUSDDebt()
    //
    ...
    //  })
    //
    //  it("adjustTrove(): changes ZKUSDToken balance by the requested decrease", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: alice}})
    //
    ...
    //      // check after
    //      const alice_ZKUSDTokenBalance_After = await zkusdToken.balanceOf(alice)
    //      assert.isTrue(alice_ZKUSDTokenBalance_After.eq(alice_ZKUSDTokenBalance_Before.sub(toBN(dec(10, 18)))))
    //  })
    //
    //  it("adjustTrove(): changes ZKUSDToken balance by the requested increase", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: alice}})
    //
    ...
    //      // check after
    //      const alice_ZKUSDTokenBalance_After = await zkusdToken.balanceOf(alice)
    //      assert.isTrue(alice_ZKUSDTokenBalance_After.eq(alice_ZKUSDTokenBalance_Before.add(toBN(dec(100, 18)))))
    //  })
    //
    ...
    //  })
    //
    //  it("adjustTrove(): Changes the ZKUSD debt in ActivePool by requested decrease", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: alice}})
    //
    //      const activePool_ZKUSDDebt_Before = await activePool.getZKUSDDebt()
    //      assert.isTrue(activePool_ZKUSDDebt_Before.gt(toBN('0')))
    //
    ...
    //      assert.isTrue(activePool_ZKUSDDebt_After.eq(activePool_ZKUSDDebt_Before.sub(toBN(dec(30, 18)))))
    //  })
    //
    //  it("adjustTrove(): Changes the ZKUSD debt in ActivePool by requested increase", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: alice}})
    //
    //      const activePool_ZKUSDDebt_Before = await activePool.getZKUSDDebt()
    ...
    //      const activePool_ZKUSDDebt_After = await activePool.getZKUSDDebt()
    //
    //      th.assertIsApproximatelyEqual(activePool_ZKUSDDebt_After, activePool_ZKUSDDebt_Before.add(toBN(dec(100, 18))))
    //  })
    //
    ...
    //  })
    //
    //  it("adjustTrove(): Reverts if borrower has insufficient ZKUSD balance to cover his debt repayment", async () => {
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: whale}})
    //      await openTrove({extraZKUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: {from: B}})
    //      const bobDebt = await getTroveEntireDebt(B)
    //
    //      // Bob transfers some ZKUSD to carol
    //      await zkusdToken.transfer(C, dec(10, 18), {from: B})
    //
    //      //Confirm B's ZKUSD balance is less than 50 ZKUSD
    //      const B_ZKUSDBal = await zkusdToken.balanceOf(B)
    //      assert.isTrue(B_ZKUSDBal.lt(bobDebt))
    //
    //      const repayZKUSDPromise_B = borrowerOperations.adjustTrove(th._100pct, 0, bobDebt, false, B, B, {from: B})
    //
    //      // B attempts to repay all his debt
    //      await assertRevert(repayZKUSDPromise_B, "revert")
    //  })
    //
    ...
    //
    //          // D withdraws ZKUSD
    //          const openTroveTx = await borrowerOperations.openTrove(th._100pct, D_ZKUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, {
    //              from: D,
    //              value: dec(200, 'ether')
    //          })
    //
    ...
    //      await borrowerOperations.openTrove(th._100pct, ZKUSDRequest_D, D, D, {from: D, value: dec(500, 'ether')})
    //
    //      // Check ZKT staking ZKUSD balance has increased
    //      const zktStaking_ZKUSDBalance_After = await zkusdToken.balanceOf(zktStaking.address)
    //      assert.isTrue(zktStaking_ZKUSDBalance_After.gt(zktStaking_ZKUSDBalance_Before))
    ...
    //      const txC = await borrowerOperations.openTrove(th._100pct, ZKUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, {
    //          value: dec(100, 'ether'),
    //          from: C
    //      })
    ...
    //      assert.equal(debt_Before, 0)
    //
    //      await borrowerOperations.openTrove(th._100pct, await getOpenTroveZKUSDAmount(dec(10000, 18)), alice, alice, {
    //          from: alice,
    //          value: dec(100, 'ether')
    //      })
    //
    ...
    //          await borrowerOperations.openTrove(th._100pct, troveZKUSDAmount, alice, alice, {
    //              from: alice,
    //              value: troveColl
    //          })
    //          await borrowerOperations.openTrove(th._100pct, troveZKUSDAmount, bob, bob, {from: bob, value: troveColl})
    //
    //          await priceFeed.setPrice(dec(100, 18))
    //
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdrawZKUSD, repayZKUSD, etc. Can split them up and put them with
