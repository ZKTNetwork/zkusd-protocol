...
contract("TroveManager - in Recovery Mode", async (accounts) => {
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 ZKUSD of debt, bringing his ICR to 210%
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 ZKUSD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(150, 16)),
      extraParams: { from: alice },
    });
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // check pool ZKUSD before liquidation
    const stabilityPoolZKUSD_Before = (
      await stabilityPool.getTotalZKUSDDeposits()
    ).toString();
    assert.equal(stabilityPoolZKUSD_Before, "390000000000000000000");
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 ZKUSD.
    
    With 390 ZKUSD in the StabilityPool, 390 ZKUSD should be offset with the pool, leaving 0 in the pool.
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool ZKUSD is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZKUSD > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 ZKUSD, Alice sole depositor.
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool ZKUSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(220, 16)),
      extraZKUSDAmount: dec(250, 18),
      extraParams: { from: bob },
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ZKUSD > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ZKUSD > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZKUSD > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ZKUSD > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 ZKUSD, CR of 200%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    });
  ...
  ZKUSD is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool ZKUSD < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraZKUSDAmount: dec(1500, 18),
      extraParams: { from: alice },
    });
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(240, 16)),
    ...
    /* Since the pool only contains 100 ZKUSD, and Bob's pre-liquidation debt was 250 ZKUSD,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZKUSD < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZKUSD < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 ZKUSD) is larger than all ZKUSD in the Stability Pool, Liquidation won’t happen
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZKUSD < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ZKUSD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    ...
    // Alice withdraws up to 1500 ZKUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 ZKUSD, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraZKUSDAmount: dec(1500, 18),
      extraParams: { from: alice },
    ...
    //Check SP ZKUSD has been completely emptied
    assert.equal(
    ...
    await zkusdToken.transfer(bob, dec(100, 18), { from: alice });
  ...
  it("liquidate() with ICR > 110%, and StabilityPool ZKUSD < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 ZKUSD to the SP
    await openTrove({
      ICR: toBN(dec(300, 16)),
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 ZKUSD in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({
      ICR: toBN(dec(220, 16)),
      extraZKUSDAmount: dec(1000, 18),
      extraParams: { from: whale },
    });

    const { zkusdAmount: A_zkusdAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraZKUSDAmount: dec(300, 18),
      extraParams: { from: alice },
    });
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ZKUSD of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 ZKUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraZKUSDAmount: dec(480, 18),
      extraParams: { from: bob },
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 ZKUSD of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(222, 16)),
      extraZKUSDAmount: dec(90, 18),
      extraParams: { from: bob },
    });
    // Dennis withdraws to 150 ZKUSD of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 250 ZKUSD, ICR = 240% (lowest one)
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some ZKUSD from their Trove, resulting in different ICRs
    ...
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraZKUSDAmount: liquidationAmount,
      extraParams: { from: alice },
    });
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraZKUSDAmount: dec(101, 18),
      extraParams: { from: defaulter_1 },
    });
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraZKUSDAmount: dec(217, 18),
      extraParams: { from: defaulter_2 },
    });
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraZKUSDAmount: dec(328, 18),
      extraParams: { from: defaulter_3 },
    });
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraZKUSDAmount: dec(431, 18),
    ...
    // Check Stability Pool has 500 ZKUSD
    assert.equal(
      (await stabilityPool.getTotalZKUSDDeposits()).toString(),
      dec(500, 18)
    );
  ...
  it("liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty", async () => {
    ...
    // Confirm 0 ZKUSD in Stability Pool
    assert.equal((await stabilityPool.getTotalZKUSDDeposits()).toString(), "0");
  ...
  it("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
    ...
    // Confirm ZKUSD in Stability Pool
    assert.equal(
      (await stabilityPool.getTotalZKUSDDeposits()).toString(),
      spDeposit.toString()
    );
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { zkusdAmount: zkusdAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis },
    });
    const { zkusdAmount: zkusdAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin },
    });
    const { zkusdAmount: zkusdAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy },
    });
    ...
    assert.equal((await zkusdToken.balanceOf(dennis)).toString(), zkusdAmountD);
    assert.equal((await zkusdToken.balanceOf(erin)).toString(), zkusdAmountE);
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    ...
    await stabilityPool.provideToSP(W_zkusdAmount, ZERO_ADDRESS, {
      from: whale,
    });

    const {
      zkusdAmount: A_zkusdAmount,
    ...
    await stabilityPool.provideToSP(A_zkusdAmount, ZERO_ADDRESS, {
      from: alice,
    });
    await stabilityPool.provideToSP(B_zkusdAmount, ZERO_ADDRESS, { from: bob });
    ...
    Bob ZKUSD Loss: 500 * (240/680) = 176.47 ZKUSD

    Whale remaining deposit: (400 - 294.12) = 105.88 ZKUSD
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_zkusdAmount.sub(liquidatedDebt.mul(W_zkusdAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_zkusdAmount.sub(liquidatedDebt.mul(A_zkusdAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_zkusdAmount.sub(liquidatedDebt.mul(B_zkusdAmount).div(totalDeposit))
      ),
      100000
    );
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    That leaves 50 ZKUSD in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10);
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 ZKUSD in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ZKUSD in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10);

    const [liquidatedDebt, liquidatedColl, collGasComp, zkusdGasComp] =
      th.getEmittedLiquidationValues(liquidationTx);
    ...
    assert.equal(zkusdGasComp.toString(), dec(400, 18));
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool ZKUSD > debt to liquidate: can liquidate troves out of order", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    });
    ...
    // Whale provides 1000 ZKUSD to the SP
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(
      await troveManager.getPendingZKUSDDebtReward(dennis)
    );
  ...
  it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
    ...
    // Whale withdraws entire deposit, and re-deposits 132 ZKUSD
    ...
    // Expected Stability Pool to fully absorb B (92 ZKUSD + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalZKUSDDeposits();
    ...
    const stabilityAfter = await stabilityPool.getTotalZKUSDDeposits();
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    // Whale opens trove and adds 220 ZKUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(
      A_totalDebt.div(toBN(2))
    );
    await openTrove({
