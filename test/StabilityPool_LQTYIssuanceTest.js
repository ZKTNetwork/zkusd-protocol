...
contract("StabilityPool - ZKT Rewards", async (accounts) => {
  ...
  describe("ZKT Rewards", async () => {
    ...
    1 month passes. L1 decreases P: P = 1e-5 P. L1:   9999.9 ZKUSD, 100 ETH
    B makes deposit 9999.9
    1 month passes. L2 decreases P: P =  1e-5 P. L2:  9999.9 ZKUSD, 100 ETH
    C makes deposit  9999.9
    1 month passes. L3 decreases P: P = 1e-5 P. L3:  9999.9 ZKUSD, 100 ETH
    D makes deposit  9999.9
    1 month passes. L4 decreases P: P = 1e-5 P. L4:  9999.9 ZKUSD, 100 ETH
    E makes deposit  9999.9
    1 month passes. L5 decreases P: P = 1e-5 P. L5:  9999.9 ZKUSD, 100 ETH
    =========
    F makes deposit 100
    1 month passes. L6 empties the Pool. L6:  10000 ZKUSD, 100 ETH
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct ZKT gain. No liquidations. Front ends and kickback rates.", async () => {
      ...
      assert.equal(await zkToken.balanceOf(C), 0);
      assert.equal(await zkToken.balanceOf(D), 0);
      assert.equal(await zkToken.balanceOf(frontEnd_1), 0);
      assert.equal(await zkToken.balanceOf(frontEnd_2), 0);
      ...
      // E deposits, triggering ZKT gains for A,B,C,D,F1,F2. Withdraws immediately after
      await stabilityPool.provideToSP(dec(1, 18), ZERO_ADDRESS, { from: E });
      await stabilityPool.withdrawFromSP(dec(1, 18), { from: E });

      // Expected issuance for year 1 is 50% of total supply.
      const expectedIssuance_Y1 = communityZKTSupply.div(toBN("2"));

      // Get actual ZKT gains
      const A_ZKTGain_Y1 = await stabilityPool.getDepositorZKTGain(A);
      const B_ZKTGain_Y1 = await stabilityPool.getDepositorZKTGain(B);
      const C_ZKTGain_Y1 = await stabilityPool.getDepositorZKTGain(C);
      const D_ZKTGain_Y1 = await stabilityPool.getDepositorZKTGain(D);
      const F1_ZKTGain_Y1 = await stabilityPool.getFrontEndZKTGain(frontEnd_1);
      const F2_ZKTGain_Y1 = await stabilityPool.getFrontEndZKTGain(frontEnd_2);
      ...
      // Check gains are correct, error tolerance = 1e-6 of a token
      assert.isAtMost(getDifference(A_ZKTGain_Y1, A_expectedGain_Y1), 1e12);
      assert.isAtMost(getDifference(B_ZKTGain_Y1, B_expectedGain_Y1), 1e12);
      assert.isAtMost(getDifference(C_ZKTGain_Y1, C_expectedGain_Y1), 1e12);
      assert.isAtMost(getDifference(D_ZKTGain_Y1, D_expectedGain_Y1), 1e12);

      assert.isAtMost(getDifference(F1_ZKTGain_Y1, F1_expectedGain_Y1), 1e12);
      assert.isAtMost(getDifference(F2_ZKTGain_Y1, F2_expectedGain_Y1), 1e12);
      ...
      // E deposits, triggering ZKT gains for A,B,CD,F1, F2. Withdraws immediately after
      await stabilityPool.provideToSP(dec(1, 18), ZERO_ADDRESS, { from: E });
      await stabilityPool.withdrawFromSP(dec(1, 18), { from: E });

      // Expected gains for each depositor in Y2(25% total issued).  .
      const expectedIssuance_Y2 = communityZKTSupply.div(toBN("4"));
      ...
      // Check ZKT balances increase by correct amount
      assert.isAtMost(
        getDifference(await zkToken.balanceOf(A), A_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await zkToken.balanceOf(B), B_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await zkToken.balanceOf(C), C_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(await zkToken.balanceOf(D), D_expectedFinalGain),
        1e12
      );
      assert.isAtMost(
        getDifference(
          await zkToken.balanceOf(frontEnd_1),
      ...
      assert.isAtMost(
        getDifference(
          await zkToken.balanceOf(frontEnd_2),
    ...
    // 1 month passes.
    // All withdraw
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct ZKT gain. Front ends and kickback rates", async () => {
      // Register 2 front ends
      ...
      await borrowerOperations.openTrove(
        th._100pct,
        await getOpenTroveZKUSDAmount(dec(50000, 18)),
        defaulter_1,
        defaulter_1,
      ...
      const A_expectedZKTGain_M1 = F1_kickbackRate.mul(A_share_M1).div(
        toBN(dec(1, 18))
      );

      const B_share_M1 = issuance_M1.mul(toBN("20000")).div(toBN("100000"));
      const B_expectedZKTGain_M1 = F2_kickbackRate.mul(B_share_M1).div(
        toBN(dec(1, 18))
      );

      const C_share_M1 = issuance_M1.mul(toBN("30000")).div(toBN("100000"));
      const C_expectedZKTGain_M1 = F2_kickbackRate.mul(C_share_M1).div(
        toBN(dec(1, 18))
      );

      const D_share_M1 = issuance_M1.mul(toBN("40000")).div(toBN("100000"));
      const D_expectedZKTGain_M1 = D_share_M1;

      // F1's stake = A
      const F1_expectedZKTGain_M1 = toBN(dec(1, 18))
        .sub(F1_kickbackRate)
        .mul(A_share_M1)
        .div(toBN(dec(1, 18)));

      // F2's stake = B + C
      const F2_expectedZKTGain_M1 = toBN(dec(1, 18))
        .sub(F2_kickbackRate)
        .mul(B_share_M1.add(C_share_M1))
        .div(toBN(dec(1, 18)));

      // Check ZKT gain
      const A_ZKTGain_M1 = await stabilityPool.getDepositorZKTGain(A);
      const B_ZKTGain_M1 = await stabilityPool.getDepositorZKTGain(B);
      const C_ZKTGain_M1 = await stabilityPool.getDepositorZKTGain(C);
      const D_ZKTGain_M1 = await stabilityPool.getDepositorZKTGain(D);
      const F1_ZKTGain_M1 = await stabilityPool.getFrontEndZKTGain(frontEnd_1);
      const F2_ZKTGain_M1 = await stabilityPool.getFrontEndZKTGain(frontEnd_2);

      // Check gains are correct, error tolerance = 1e-3 of a token
      assert.isAtMost(getDifference(A_ZKTGain_M1, A_expectedZKTGain_M1), 1e15);
      assert.isAtMost(getDifference(B_ZKTGain_M1, B_expectedZKTGain_M1), 1e15);
      assert.isAtMost(getDifference(C_ZKTGain_M1, C_expectedZKTGain_M1), 1e15);
      assert.isAtMost(getDifference(D_ZKTGain_M1, D_expectedZKTGain_M1), 1e15);
      assert.isAtMost(
        getDifference(F1_ZKTGain_M1, F1_expectedZKTGain_M1),
        1e15
      );
      assert.isAtMost(
        getDifference(F2_ZKTGain_M1, F2_expectedZKTGain_M1),
        1e15
      );
      ...
      const A_expectedZKTGain_M2 = F1_kickbackRate.mul(A_share_M2).div(
        toBN(dec(1, 18))
      );

      const B_share_M2 = issuance_M2.mul(toBN("10000")).div(toBN("80000"));
      const B_expectedZKTGain_M2 = F2_kickbackRate.mul(B_share_M2).div(
        toBN(dec(1, 18))
      );

      const C_share_M2 = issuance_M2.mul(toBN("15000")).div(toBN("80000"));
      const C_expectedZKTGain_M2 = F2_kickbackRate.mul(C_share_M2).div(
        toBN(dec(1, 18))
      );

      const D_share_M2 = issuance_M2.mul(toBN("20000")).div(toBN("80000"));
      const D_expectedZKTGain_M2 = D_share_M2;

      const E_share_M2 = issuance_M2.mul(toBN("30000")).div(toBN("80000"));
      const E_expectedZKTGain_M2 = F1_kickbackRate.mul(E_share_M2).div(
        toBN(dec(1, 18))
      );

      // F1's stake = A + E
      const F1_expectedZKTGain_M2 = toBN(dec(1, 18))
        .sub(F1_kickbackRate)
        .mul(A_share_M2.add(E_share_M2))
        .div(toBN(dec(1, 18)));

      // F2's stake = B + C
      const F2_expectedZKTGain_M2 = toBN(dec(1, 18))
        .sub(F2_kickbackRate)
        .mul(B_share_M2.add(C_share_M2))
        .div(toBN(dec(1, 18)));

      // Check ZKT gains after month 2
      const A_ZKTGain_After_M2 = await stabilityPool.getDepositorZKTGain(A);
      const B_ZKTGain_After_M2 = await stabilityPool.getDepositorZKTGain(B);
      const C_ZKTGain_After_M2 = await stabilityPool.getDepositorZKTGain(C);
      const D_ZKTGain_After_M2 = await stabilityPool.getDepositorZKTGain(D);
      const E_ZKTGain_After_M2 = await stabilityPool.getDepositorZKTGain(E);
      const F1_ZKTGain_After_M2 = await stabilityPool.getFrontEndZKTGain(
        frontEnd_1
      );
      const F2_ZKTGain_After_M2 = await stabilityPool.getFrontEndZKTGain(
        frontEnd_2
      );
      ...
      assert.isAtMost(
        getDifference(
          A_ZKTGain_After_M2,
          A_expectedZKTGain_M2.add(A_expectedZKTGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          B_ZKTGain_After_M2,
          B_expectedZKTGain_M2.add(B_expectedZKTGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          C_ZKTGain_After_M2,
          C_expectedZKTGain_M2.add(C_expectedZKTGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          D_ZKTGain_After_M2,
          D_expectedZKTGain_M2.add(D_expectedZKTGain_M1)
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(E_ZKTGain_After_M2, E_expectedZKTGain_M2),
        1e15
      );

      // Check F1 balance is his M1 gain (it was paid out when E joined through F1)
      const F1_ZKTBalance_After_M2 = await zkToken.balanceOf(frontEnd_1);
      assert.isAtMost(
        getDifference(F1_ZKTBalance_After_M2, F1_expectedZKTGain_M1),
        1e15
      );

      // Check F1's ZKT gain in system after M2: Just their gain due to M2
      assert.isAtMost(
        getDifference(F1_ZKTGain_After_M2, F1_expectedZKTGain_M2),
        1e15
      );

      // Check F2 ZKT gain in system after M2: the sum of their gains from M1 + M2
      assert.isAtMost(
        getDifference(
          F2_ZKTGain_After_M2,
          F2_expectedZKTGain_M2.add(F2_expectedZKTGain_M1)
        ),
        1e15
      );
      ...
      const A_expectedZKTGain_M3 = F1_kickbackRate.mul(A_share_M3).div(
        toBN(dec(1, 18))
      );

      const B_share_M3 = issuance_M3.mul(toBN("47500")).div(toBN("100000"));
      const B_expectedZKTGain_M3 = F2_kickbackRate.mul(B_share_M3).div(
        toBN(dec(1, 18))
      );

      const C_share_M3 = issuance_M3.mul(toBN("11250")).div(toBN("100000"));
      const C_expectedZKTGain_M3 = F2_kickbackRate.mul(C_share_M3).div(
        toBN(dec(1, 18))
      );

      const D_share_M3 = issuance_M3.mul(toBN("15000")).div(toBN("100000"));
      const D_expectedZKTGain_M3 = D_share_M3;

      const E_share_M3 = issuance_M3.mul(toBN("22500")).div(toBN("100000"));
      const E_expectedZKTGain_M3 = F1_kickbackRate.mul(E_share_M3).div(
        toBN(dec(1, 18))
      );

      // F1's stake = A + E
      const F1_expectedZKTGain_M3 = toBN(dec(1, 18))
        .sub(F1_kickbackRate)
        .mul(A_share_M3.add(E_share_M3))
        .div(toBN(dec(1, 18)));

      // F2's stake = B + C
      const F2_expectedZKTGain_M3 = toBN(dec(1, 18))
        .sub(F2_kickbackRate)
        .mul(B_share_M3.add(C_share_M3))
        .div(toBN(dec(1, 18)));

      // Check ZKT gains after month 3
      const A_ZKTGain_After_M3 = await stabilityPool.getDepositorZKTGain(A);
      const B_ZKTGain_After_M3 = await stabilityPool.getDepositorZKTGain(B);
      const C_ZKTGain_After_M3 = await stabilityPool.getDepositorZKTGain(C);
      const D_ZKTGain_After_M3 = await stabilityPool.getDepositorZKTGain(D);
      const E_ZKTGain_After_M3 = await stabilityPool.getDepositorZKTGain(E);
      const F1_ZKTGain_After_M3 = await stabilityPool.getFrontEndZKTGain(
        frontEnd_1
      );
      const F2_ZKTGain_After_M3 = await stabilityPool.getFrontEndZKTGain(
        frontEnd_2
      );

      // Expect A, C, D ZKT system gains to equal their gains from (M1 + M2 + M3)
      assert.isAtMost(
        getDifference(
          A_ZKTGain_After_M3,
          A_expectedZKTGain_M3.add(A_expectedZKTGain_M2).add(
            A_expectedZKTGain_M1
          )
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          C_ZKTGain_After_M3,
          C_expectedZKTGain_M3.add(C_expectedZKTGain_M2).add(
            C_expectedZKTGain_M1
          )
        ),
        1e15
      );
      assert.isAtMost(
        getDifference(
          D_ZKTGain_After_M3,
          D_expectedZKTGain_M3.add(D_expectedZKTGain_M2).add(
            D_expectedZKTGain_M1
          )
        ),
        1e15
      );

      // Expect E's ZKT system gain to equal their gains from (M2 + M3)
      assert.isAtMost(
        getDifference(
          E_ZKTGain_After_M3,
          E_expectedZKTGain_M3.add(E_expectedZKTGain_M2)
        ),
        1e15
      );

      // Expect B ZKT system gains to equal gains just from M3 (his topup paid out his gains from M1 + M2)
      assert.isAtMost(
        getDifference(B_ZKTGain_After_M3, B_expectedZKTGain_M3),
        1e15
      );

      // Expect B ZKT balance to equal gains from (M1 + M2)
      const B_ZKTBalance_After_M3 = await await zkToken.balanceOf(B);
      assert.isAtMost(
        getDifference(
          B_ZKTBalance_After_M3,
          B_expectedZKTGain_M2.add(B_expectedZKTGain_M1)
        ),
        1e15
      );

      // Expect F1 ZKT system gains to equal their gain from (M2 + M3)
      assert.isAtMost(
        getDifference(
          F1_ZKTGain_After_M3,
          F1_expectedZKTGain_M3.add(F1_expectedZKTGain_M2)
        ),
        1e15
      );

      // Expect F1 ZKT balance to equal their M1 gain
      const F1_ZKTBalance_After_M3 = await zkToken.balanceOf(frontEnd_1);
      assert.isAtMost(
        getDifference(F1_ZKTBalance_After_M3, F1_expectedZKTGain_M1),
        1e15
      );

      // Expect F2 ZKT system gains to equal their gain from M3
      assert.isAtMost(
        getDifference(F2_ZKTGain_After_M3, F2_expectedZKTGain_M3),
        1e15
      );

      // Expect F2 ZKT balance to equal their gain from M1 + M2
      const F2_ZKTBalance_After_M3 = await zkToken.balanceOf(frontEnd_2);
      assert.isAtMost(
        getDifference(
          F2_ZKTBalance_After_M3,
          F2_expectedZKTGain_M2.add(F2_expectedZKTGain_M1)
        ),
        1e15
      );

      // Expect deposit C now to be 10125 ZKUSD
      const C_compoundedZKUSDDeposit =
        await stabilityPool.getCompoundedZKUSDDeposit(C);
      assert.isAtMost(
        getDifference(C_compoundedZKUSDDeposit, dec(10125, 18)),
        1000
      );
      ...
      for (depositor of [A, B, C, D, E]) {
        ...
        const compoundedZKUSDDeposit =
          await stabilityPool.getCompoundedZKUSDDeposit(depositor);
        assert.equal(compoundedZKUSDDeposit, "0");
      }
      ...
      const A_expectedZKTGain_M4 = F1_kickbackRate.mul(A_share_M4).div(
        toBN(dec(1, 18))
      );

      const B_share_M4 = issuance_M4.mul(toBN("42750")).div(toBN("80000")); // 42750/80000
      const B_expectedZKTGain_M4 = F2_kickbackRate.mul(B_share_M4).div(
        toBN(dec(1, 18))
      );

      const C_share_M4 = issuance_M4.mul(toBN("125")).div(toBN("80000")); // 125/80000
      const C_expectedZKTGain_M4 = F2_kickbackRate.mul(C_share_M4).div(
        toBN(dec(1, 18))
      );

      const D_share_M4 = issuance_M4.mul(toBN("13500")).div(toBN("80000"));
      const D_expectedZKTGain_M4 = D_share_M4;

      const E_share_M4 = issuance_M4.mul(toBN("20250")).div(toBN("80000")); // 2025/80000
      const E_expectedZKTGain_M4 = F1_kickbackRate.mul(E_share_M4).div(
        toBN(dec(1, 18))
      );

      // F1's stake = A + E
      const F1_expectedZKTGain_M4 = toBN(dec(1, 18))
        .sub(F1_kickbackRate)
        .mul(A_share_M4.add(E_share_M4))
        .div(toBN(dec(1, 18)));

      // F2's stake = B + C
      const F2_expectedZKTGain_M4 = toBN(dec(1, 18))
        .sub(F2_kickbackRate)
        .mul(B_share_M4.add(C_share_M4))
        .div(toBN(dec(1, 18)));

      // Get final ZKT balances
      const A_FinalZKTBalance = await zkToken.balanceOf(A);
      const B_FinalZKTBalance = await zkToken.balanceOf(B);
      const C_FinalZKTBalance = await zkToken.balanceOf(C);
      const D_FinalZKTBalance = await zkToken.balanceOf(D);
      const E_FinalZKTBalance = await zkToken.balanceOf(E);
      const F1_FinalZKTBalance = await zkToken.balanceOf(frontEnd_1);
      const F2_FinalZKTBalance = await zkToken.balanceOf(frontEnd_2);

      const A_expectedFinalZKTBalance = A_expectedZKTGain_M1.add(
        A_expectedZKTGain_M2
      )
        .add(A_expectedZKTGain_M3)
        .add(A_expectedZKTGain_M4);

      const B_expectedFinalZKTBalance = B_expectedZKTGain_M1.add(
        B_expectedZKTGain_M2
      )
        .add(B_expectedZKTGain_M3)
        .add(B_expectedZKTGain_M4);

      const C_expectedFinalZKTBalance = C_expectedZKTGain_M1.add(
        C_expectedZKTGain_M2
      )
        .add(C_expectedZKTGain_M3)
        .add(C_expectedZKTGain_M4);

      const D_expectedFinalZKTBalance = D_expectedZKTGain_M1.add(
        D_expectedZKTGain_M2
      )
        .add(D_expectedZKTGain_M3)
        .add(D_expectedZKTGain_M4);

      const E_expectedFinalZKTBalance =
        E_expectedZKTGain_M2.add(E_expectedZKTGain_M3).add(
          E_expectedZKTGain_M4
        );

      const F1_expectedFinalZKTBalance = F1_expectedZKTGain_M1.add(
        F1_expectedZKTGain_M2
      )
        .add(F1_expectedZKTGain_M3)
        .add(F1_expectedZKTGain_M4);

      const F2_expectedFinalZKTBalance = F2_expectedZKTGain_M1.add(
        F2_expectedZKTGain_M2
      )
        .add(F2_expectedZKTGain_M3)
        .add(F2_expectedZKTGain_M4);

      assert.isAtMost(
        getDifference(A_FinalZKTBalance, A_expectedFinalZKTBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(B_FinalZKTBalance, B_expectedFinalZKTBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(C_FinalZKTBalance, C_expectedFinalZKTBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(D_FinalZKTBalance, D_expectedFinalZKTBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(E_FinalZKTBalance, E_expectedFinalZKTBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(F1_FinalZKTBalance, F1_expectedFinalZKTBalance),
        1e15
      );
      assert.isAtMost(
        getDifference(F2_FinalZKTBalance, F2_expectedFinalZKTBalance),
        1e15
      );
    });
    ...
    A, B make deposit 5000 ZKUSD via F1
    1 month passes. L1 depletes P: P = 1e-5*P L1:  9999.9 ZKUSD, 1 ETH.  scale = 0
    C makes deposit 10000  via F1
    1 month passes. L2 depletes P: P = 1e-5*P L2:  9999.9 ZKUSD, 1 ETH  scale = 1
    D makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L3:  9999.9 ZKUSD, 1 ETH scale = 1
    E makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L4:  9999.9 ZKUSD, 1 ETH scale = 2
    A, B, C, D, E withdraw

    =========
    Expect front end withdraws ~3 month's worth of ZKT */

    it("withdrawFromSP(): Several deposits of 10k ZKUSD span one scale factor change. Depositors withdraw correct ZKT gains", async () => {
      ...
      // Confirm all would-be depositors have 0 ZKT
      ...
      const ZKTGain_F1 = await zkToken.balanceOf(frontEnd_1);

      /* Expect each deposit to have earned ZKT issuance for the month in which it was active, prior
     to the liquidation that mostly depleted it:
     
     expectedZKTGain_A:  (k * M1 / 2) + (k * M2 / 2) / 100000   
     expectedZKTGain_B:  (k * M1 / 2) + (k * M2 / 2) / 100000                           

     expectedZKTGain_C:  ((k * M2)  + (k * M3) / 100000) * 9999.9/10000   
     expectedZKTGain_D:  ((k * M3)  + (k * M4) / 100000) * 9999.9/10000 
     expectedZKTGain_E:  (k * M4) * 9999.9/10000 

     expectedZKTGain_F1:  (1 - k) * (M1 + M2 + M3 + M4)
     */

      const expectedZKTGain_A_and_B = kickbackRate
        .mul(issuance_M1)
        .div(toBN("2"))
      ...
      const expectedZKTGain_C = kickbackRate
        .mul(issuance_M2)
      ...
      const expectedZKTGain_D = kickbackRate
        .mul(issuance_M3)
      ...
      const expectedZKTGain_E = kickbackRate
        .mul(issuance_M4)
      ...
      const issuance1st4Months = issuance_M1
        ...
        .add(issuance_M3)
        .add(issuance_M4);
      const expectedZKTGain_F1 = toBN(dec(1, 18))
        .sub(kickbackRate)
        .mul(issuance1st4Months)
        .div(toBN(dec(1, 18)));

      assert.isAtMost(getDifference(expectedZKTGain_A_and_B, ZKTGain_A), 1e15);
      assert.isAtMost(getDifference(expectedZKTGain_A_and_B, ZKTGain_B), 1e15);
      assert.isAtMost(getDifference(expectedZKTGain_C, ZKTGain_C), 1e15);
      assert.isAtMost(getDifference(expectedZKTGain_D, ZKTGain_D), 1e15);
      assert.isAtMost(getDifference(expectedZKTGain_E, ZKTGain_E), 1e15);
      assert.isAtMost(getDifference(expectedZKTGain_F1, ZKTGain_F1), 1e15);
    });
  });
});
