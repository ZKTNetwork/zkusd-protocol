...
contract("TroveManager", async (accounts) => {
  ...
  const getOpenTroveTotalDebt = async (zkusdAmount) =>
    th.getOpenTroveTotalDebt(contracts, zkusdAmount);
  ...
  const withdrawZKUSD = async (params) => th.withdrawZKUSD(contracts, params);
  ...
  it("liquidate(): closes a Trove that has ICR < MCR", async () => {
    ...
    const A_ZKUSDWithdrawal = await getNetBorrowingAmount(dec(130, 18));

    const targetICR = toBN("1111111111111111111");
    await withdrawZKUSD({ ICR: targetICR, extraParams: { from: alice } });
  ...
  it("liquidate(): decreases ActivePool NEON and ZKUSDDebt by correct amounts", async () => {
    ...
    const activePool_NEON_Before = (await activePool.getNEON()).toString();
    const activePool_RawEther_Before = (
      await web3.eth.getBalance(activePool.address)
    ).toString();
    const activePool_ZKUSDDebt_Before = (
      await activePool.getZKUSDDebt()
    ).toString();

    assert.equal(activePool_NEON_Before, A_collateral.add(B_collateral));
    ...
    const activePool_NEON_After = (await activePool.getNEON()).toString();
    const activePool_RawEther_After = (
      await web3.eth.getBalance(activePool.address)
    ).toString();
    const activePool_ZKUSDDebt_After = (
      await activePool.getZKUSDDebt()
    ).toString();

    assert.equal(activePool_NEON_After, A_collateral);
  ...
  it("liquidate(): increases DefaultPool NEON and ZKUSD debt by correct amounts", async () => {
    ...
    const defaultPool_ZKUSDDebt_Before = (
      await defaultPool.getZKUSDDebt()
    ).toString();

    assert.equal(defaultPool_NEON_Before, "0");
    assert.equal(defaultPool_RawEther_Before, "0");
    assert.equal(defaultPool_ZKUSDDebt_Before, "0");
    ...
    // check after
    const defaultPool_NEON_After = (await defaultPool.getNEON()).toString();
    const defaultPool_RawEther_After = (
      await web3.eth.getBalance(defaultPool.address)
    ).toString();
    const defaultPool_ZKUSDDebt_After = (
      await defaultPool.getZKUSDDebt()
    ).toString();

    const defaultPool_NEON = th.applyLiquidationFee(B_collateral);
    assert.equal(defaultPool_NEON_After, defaultPool_NEON);
    assert.equal(defaultPool_RawEther_After, defaultPool_NEON);
  ...
  it("liquidate(): updates the L_NEON and L_ZKUSDDebt reward-per-unit-staked totals", async () => {
    ...
    const L_NEON_AfterCarolLiquidated = await troveManager.L_NEON();
    const L_ZKUSDDebt_AfterCarolLiquidated = await troveManager.L_ZKUSDDebt();
   ...
   L_ZKUSDDebt = (180 / 20) + (890 / 10) = 98 ZKUSD */
    const L_NEON_AfterBobLiquidated = await troveManager.L_NEON();
    const L_ZKUSDDebt_AfterBobLiquidated = await troveManager.L_ZKUSDDebt();
    ...
    assert.isAtMost(
      th.getDifference(L_NEON_AfterBobLiquidated, L_NEON_expected_2),
      100
    );
    assert.isAtMost(
  ...
  it("liquidate(): does not affect the SP deposit or NEON gain when called on an SP depositor's address that has no trove", async () => {
    ...
    assert.isAtMost(
      th.getDifference(dennis_NEONGain_Before, liquidatedColl),
      1000
    );
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or NEON gain", async () => {
    ...
    assert.isAtMost(
      th.getDifference(bob_NEONGain_Before, liquidatedColl),
      1000
    );
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and NEON gain", async () => {
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedZKUSDDeposit(
      bob
    );
    const bob_NEONGain_After = await stabilityPool.getDepositorNEONGain(bob);
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
    const { zkusdAmount: A_zkusdAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraZKUSDAmount: toBN(dec(300, 18)),
      extraParams: { from: alice },
    });
    const { zkusdAmount: B_zkusdAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraZKUSDAmount: toBN(dec(200, 18)),
      extraParams: { from: bob },
    });
    const { zkusdAmount: C_zkusdAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraZKUSDAmount: toBN(dec(100, 18)),
      extraParams: { from: carol },
    });
    ...
    assert.equal((await zkusdToken.balanceOf(alice)).toString(), A_zkusdAmount);
    assert.equal((await zkusdToken.balanceOf(bob)).toString(), B_zkusdAmount);
    assert.equal((await zkusdToken.balanceOf(carol)).toString(), C_zkusdAmount);
  });
  ...
  it("liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
    ...
    await borrowerOperations.repayZKUSD(dec(1, 18), D, D, { from: D });
    await borrowerOperations.repayZKUSD(dec(1, 18), E, E, { from: E });
    ...
    const pendingNEON_C = await troveManager.getPendingNEONReward(C);
    const pendingZKUSDDebt_C = await troveManager.getPendingZKUSDDebtReward(C);
    const defaultPoolNEON = await defaultPool.getNEON();
    const defaultPoolZKUSDDebt = await defaultPool.getZKUSDDebt();
    assert.isTrue(pendingNEON_C.lte(defaultPoolNEON));
    assert.isTrue(pendingZKUSDDebt_C.lte(defaultPoolZKUSDDebt));
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingNEON_C, defaultPoolNEON), 1000);
    assert.isAtMost(
      th.getDifference(pendingZKUSDDebt_C, defaultPoolZKUSDDebt),
      1000
    );
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await zkusdToken.balanceOf(dennis);
    const E_balanceBefore = await zkusdToken.balanceOf(erin);
    const F_balanceBefore = await zkusdToken.balanceOf(flyn);
    ...
    assert.equal(
      (await zkusdToken.balanceOf(erin)).toString(),
      E_balanceBefore
    );
    assert.equal(
      (await zkusdToken.balanceOf(flyn)).toString(),
      F_balanceBefore
    );
  });
  ...
  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    assert.equal(
      (await stabilityPool.getTotalZKUSDDeposits()).toString(),
      dec(500, 18)
    );
    ...
    assert.equal((await stabilityPool.getTotalZKUSDDeposits()).toString(), "0");
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and NEON gain", async () => {
    ...
    const whale_Deposit_After = await stabilityPool.getCompoundedZKUSDDeposit(
      whale
    );
    const alice_Deposit_After = await stabilityPool.getCompoundedZKUSDDeposit(
      alice
    );
    const bob_Deposit_After = await stabilityPool.getCompoundedZKUSDDeposit(
      bob
    ...
    const whale_NEONGain = await stabilityPool.getDepositorNEONGain(whale);
    const alice_NEONGain = await stabilityPool.getDepositorNEONGain(alice);
    const bob_NEONGain = await stabilityPool.getDepositorNEONGain(bob);
  ...
  it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
    ...
    // Price drops to 1NEON:100ZKUSD, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("batchLiquidateTroves: when SP > 0, triggers ZKT reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } });
  ...
  it("getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraZKUSDAmount: partialRedemptionAmount,
      extraParams: { from: alice },
    });
  ...
  it("getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations", async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra ZKUSD. At least 3 iterations are needed
  ...
  it("redeemCollateral(): cancels the provided ZKUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
    // --- SETUP ---
    ...
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraZKUSDAmount: redemptionAmount,
      extraParams: { from: dennis },
    });

    const dennis_NEONBalance_Before = toBN(await web3.eth.getBalance(dennis));

    const dennis_ZKUSDBalance_Before = await zkusdToken.balanceOf(dennis);
    ...
    // Find hints for redeeming 20 ZKUSD
    const { firstRedemptionHint, partialRedemptionHintNICR } =
      await hintHelpers.getRedemptionHints(redemptionAmount, price, 0);
    ...
    const NEONFee = th.getEmittedRedemptionValues(redemptionTx)[3];
    ...
    /* check that Dennis' redeemed 20 ZKUSD has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) ZKUSD debt + 50 for gas compensation. */
    th.assertIsApproximatelyEqual(
      alice_debt_After,
    ...
    const dennis_NEONBalance_After = toBN(await web3.eth.getBalance(dennis));
    const receivedNEON = dennis_NEONBalance_After.sub(
      dennis_NEONBalance_Before
    );

    const expectedTotalNEONDrawn = redemptionAmount.div(toBN(200)); // convert redemptionAmount ZKUSD to NEON, at NEON:USD price 200
    const expectedReceivedNEON = expectedTotalNEONDrawn
      .sub(toBN(NEONFee))
      .sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)); // substract gas used for troveManager.redeemCollateral from expected received NEON

    // console.log("*********************************************************************************")
    // console.log("NEONFee: " + NEONFee)
    // console.log("dennis_NEONBalance_Before: " + dennis_NEONBalance_Before)
    // console.log("GAS_USED: " + th.gasUsed(redemptionTx))
    // console.log("dennis_NEONBalance_After: " + dennis_NEONBalance_After)
    // console.log("expectedTotalNEONDrawn: " + expectedTotalNEONDrawn)
    // console.log("recived  : " + receivedNEON)
    // console.log("expected : " + expectedReceivedNEON)
    // console.log("wanted :   " + expectedReceivedNEON.sub(toBN(GAS_PRICE)))
    // console.log("*********************************************************************************")
    th.assertIsApproximatelyEqual(expectedReceivedNEON, receivedNEON);

    const dennis_ZKUSDBalance_After = (
      await zkusdToken.balanceOf(dennis)
    ).toString();
    assert.equal(
      dennis_ZKUSDBalance_After,
      dennis_ZKUSDBalance_Before.sub(redemptionAmount)
    );
  });
  ...
  it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
    ...
    // open trove from redeemer.  Redeemer has highest ICR (100NEON, 100 ZKUSD), 20000%
    const { zkusdAmount: F_zkusdAmount } = await openTrove({
      ICR: toBN(dec(200, 18)),
      extraZKUSDAmount: redemptionAmount.mul(toBN(2)),
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 ZKUSD
    const flynBalance = await zkusdToken.balanceOf(flyn);
    th.assertIsApproximatelyEqual(
      flynBalance,
      F_zkusdAmount.sub(redemptionAmount)
    );
  ...
  it("redeemCollateral(): ends the redemption sequence when max iterations have been reached", async () => {
    ...
    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 ZKUSD
    const flynBalance = (await zkusdToken.balanceOf(flyn)).toString();
    th.assertIsApproximatelyEqual(
      flynBalance,
      F_zkusdAmount.sub(redemptionAmount)
    );
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    ...
    await zkusdToken.transfer(B, await zkusdToken.balanceOf(A), { from: A });
    await zkusdToken.transfer(B, await zkusdToken.balanceOf(C), { from: C });
    ...
    // ZKUSD redemption is 55000 US
    const ZKUSDRedemption = dec(55000, 18);
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    ...
    // ZKUSD redemption is 55000 ZKUSD
    const ZKUSDRedemption = dec(55000, 18);
    const tx1 = await th.redeemCollateralAndGetTxObject(
      B,
      contracts,
  ...
  it("redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date", async () => {
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 ZKUSD from Carol's Trove
      await troveManager.redeemCollateral(
        frontRunRedepmtion,
        firstRedemptionHint,
        upperPartialRedemptionHint,
    ...
    // Dennis tries to redeem 20 ZKUSD
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    // Since Alice already redeemed 1 ZKUSD from Carol's Trove, Dennis was  able to redeem:
    //  - 9 ZKUSD from Carol's
    //  - 8 ZKUSD from Bob's
    // for a total of 17 ZKUSD.

    // Dennis calculated his hint for redeeming 2 ZKUSD from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 ZKUSD to fully complete his redemption of 20 ZKUSD.
    ...
    // Expect only 17 worth of NEON drawn
    const expectedTotalNEONDrawn = fullfilledRedemptionAmount
      .sub(frontRunRedepmtion)
      .div(toBN(200)); // redempted ZKUSD converted to NEON, at NEON:USD price 200
    const expectedReceivedNEON = expectedTotalNEONDrawn
      .sub(NEONFee)
      .sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)); // substract gas used for troveManager.redeemCollateral from expected received NEON
    ...
    th.assertIsApproximatelyEqual(
      dennis_ZKUSDBalance_After,
      dennis_ZKUSDBalance_Before.sub(
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveZKUSDAmount(dec(110, 18));
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } });
    await openTrove({
      ICR: toBN(dec(133, 16)),
      extraZKUSDAmount: amount,
      extraParams: { from: bob },
    });

    await zkusdToken.transfer(carol, amount, { from: bob });
    ...
    const carol_NEONBalance_Before = toBN(await web3.eth.getBalance(carol));
    ...
    const carol_NEONBalance_After = toBN(await web3.eth.getBalance(carol));

    const expectedTotalNEONDrawn = toBN(amount).div(toBN(100)); // convert 100 ZKUSD to NEON at NEON:USD price of 100
    const expectedReceivedNEON = expectedTotalNEONDrawn.sub(NEONFee);

    const receivedNEON = carol_NEONBalance_After.sub(carol_NEONBalance_Before);
    assert.isTrue(expectedReceivedNEON.eq(receivedNEON));

    const carol_ZKUSDBalance_After = (
...
*/
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { zkusdAmount: B_zkusdAmount, totalDebt: B_totalDebt } =
      await openTrove({
        ICR: toBN(dec(133, 16)),
        extraZKUSDAmount: A_debt,
        extraParams: { from: bob },
      });

    await zkusdToken.transfer(carol, B_zkusdAmount, { from: bob });
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500ZKUSD to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraZKUSDAmount: dec(500, 18),
      extraParams: { from: alice },
    });
    await zkusdToken.transfer(erin, dec(500, 18), { from: alice });
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraZKUSDAmount: dec(30, 18),
      extraParams: { from: C },
    });
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraZKUSDAmount: dec(40, 18),
      extraParams: { from: D },
    });
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraZKUSDAmount: dec(80, 18),
      extraParams: { from: A },
    });
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraZKUSDAmount: dec(90, 18),
      extraParams: { from: B },
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
    ...
    // ZKUSD redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedZKUSDRedemption = expectedTotalSupply.div(toBN(10));
  ...
  skip("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    ...
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(395, 16)),
      extraZKUSDAmount: dec(9000, 18),
      extraParams: { from: B },
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(390, 16)),
      extraZKUSDAmount: dec(10000, 18),
      extraParams: { from: C },
    });
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt);
    ...
    // ZKUSD redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedZKUSDRedemption = expectedTotalSupply.div(toBN(10));

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice();
    const NEONDrawn = attemptedZKUSDRedemption.mul(mv._1e18BN).div(price);
    ...
    // Attempt with maxFee = 5.5%
    const exactSameFee = await troveManager.getRedemptionFeeWithDecay(
      NEONDrawn
    );
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or NEON gain of redeemed-from troves", async () => {
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } });
    ...
    const redemptionAmount = B_totalDebt.add(C_totalDebt)
      .add(D_totalDebt)
      .add(F_totalDebt);
    // Alice opens trove and transfers ZKUSD to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraZKUSDAmount: redemptionAmount,
      extraParams: { from: alice },
    ...
    // Check the remaining ZKUSD and NEON in Stability Pool after liquidation is non-zero
    const ZKUSDinSP = await stabilityPool.getTotalZKUSDDeposits();
    const NEONinSP = await stabilityPool.getNEON();
    assert.isTrue(ZKUSDinSP.gte(mv._zeroBN));
    assert.isTrue(NEONinSP.gte(mv._zeroBN));
    ...
    // Check B, C, D Stability Pool deposits and NEON gain have not been affected by redemptions from their troves
  ...
  it("redeemCollateral(): caller can redeem their entire ZKUSDToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    });

    // Alice opens trove and transfers 400 ZKUSD to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraZKUSDAmount: dec(400, 18),
      extraParams: { from: alice },
    });
    await zkusdToken.transfer(erin, dec(400, 18), { from: alice });

    // Check Erin's balance before
    const erin_balance_before = await zkusdToken.balanceOf(erin);
    assert.equal(erin_balance_before, dec(400, 18));

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraZKUSDAmount: dec(590, 18),
      extraParams: { from: bob },
    });
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraZKUSDAmount: dec(1990, 18),
      extraParams: { from: carol },
    });
    ...
    const activePool_coll_before = await activePool.getNEON();
    ...
    // Erin attempts to redeem 400 ZKUSD
    const { firstRedemptionHint, partialRedemptionHintNICR } =
    ...
    // Check activePool debt reduced by  400 ZKUSD
    const activePool_debt_after = await activePool.getZKUSDDebt();
    assert.equal(
      activePool_debt_before.sub(activePool_debt_after),
      dec(400, 18)
    );

    /* Check ActivePool coll reduced by $400 worth of Ether: at NEON:USD price of $200, this should be 2 NEON.

    therefore remaining ActivePool NEON should be 198 */
    const activePool_coll_after = await activePool.getNEON();
    ...
    const erin_balance_after = (await zkusdToken.balanceOf(erin)).toString();
    assert.equal(erin_balance_after, "0");
  });

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's ZKUSD token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    });
    ...
    // Get active debt and coll before redemption
    const activePool_debt_before = await activePool.getZKUSDDebt();
    const activePool_coll_before = (await activePool.getNEON()).toString();
    ...
    // Erin tries to redeem 1000 ZKUSD
    try {
      ...
      assert.include(
        error.message,
        "Requested redemption amount must be <= user's ZKUSD token balance"
      );
    }

    // Erin tries to redeem 401 ZKUSD
    try {
      ({ firstRedemptionHint, partialRedemptionHintNICR } =
    ...
    // Erin tries to redeem 239482309 ZKUSD
    try {
      ({ firstRedemptionHint, partialRedemptionHintNICR } =
        await hintHelpers.getRedemptionHints(
    ...
    // Erin tries to redeem 2^256 - 1 ZKUSD
  ...
  it("redeemCollateral(): value of issued NEON == face value of redeemed ZKUSD (assuming 1 ZKUSD has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale },
    });

    // Alice opens trove and transfers 1000 ZKUSD each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraZKUSDAmount: dec(4990, 18),
      extraParams: { from: alice },
    });
    await zkusdToken.transfer(erin, dec(1000, 18), { from: alice });
    await zkusdToken.transfer(flyn, dec(1000, 18), { from: alice });
    await zkusdToken.transfer(graham, dec(1000, 18), { from: alice });

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraZKUSDAmount: dec(1590, 18),
      extraParams: { from: bob },
    });
    const { collateral: C_coll } = await openTrove({
      ICR: toBN(dec(600, 16)),
      extraZKUSDAmount: dec(1090, 18),
      extraParams: { from: carol },
    });
    ...
    const _120_ZKUSD = "120000000000000000000";
    const _373_ZKUSD = "373000000000000000000";
    const _950_ZKUSD = "950000000000000000000";

    // Check Ether in activePool
    const activeNEON_0 = await activePool.getNEON();
    assert.equal(activeNEON_0, totalColl.toString());

    let firstRedemptionHint;
    let partialRedemptionHintNICR;

    // Erin redeems 120 ZKUSD
    ({ firstRedemptionHint, partialRedemptionHintNICR } =
      await hintHelpers.getRedemptionHints(_120_ZKUSD, price, 0));
    ...
    /* 120 ZKUSD redeemed.  Expect $120 worth of NEON removed. At NEON:USD price of $200,
    NEON removed = (120/200) = 0.6 NEON
    Total active NEON = 280 - 0.6 = 279.4 NEON */

    const activeNEON_1 = await activePool.getNEON();
    assert.equal(
      activeNEON_1.toString(),
      activeNEON_0.sub(toBN(_120_ZKUSD).mul(mv._1e18BN).div(price))
    );

    // Flyn redeems 373 ZKUSD
    ({ firstRedemptionHint, partialRedemptionHintNICR } =
      await hintHelpers.getRedemptionHints(_373_ZKUSD, price, 0));
    ...
    /* 373 ZKUSD redeemed.  Expect $373 worth of NEON removed. At NEON:USD price of $200,
    NEON removed = (373/200) = 1.865 NEON
    Total active NEON = 279.4 - 1.865 = 277.535 NEON */
    const activeNEON_2 = await activePool.getNEON();
    assert.equal(
      activeNEON_2.toString(),
      activeNEON_1.sub(toBN(_373_ZKUSD).mul(mv._1e18BN).div(price))
    );

    // Graham redeems 950 ZKUSD
    ({ firstRedemptionHint, partialRedemptionHintNICR } =
      await hintHelpers.getRedemptionHints(_950_ZKUSD, price, 0));
    ...
    /* 950 ZKUSD redeemed.  Expect $950 worth of NEON removed. At NEON:USD price of $200,
    NEON removed = (950/200) = 4.75 NEON
    Total active NEON = 277.535 - 4.75 = 272.785 NEON */
    const activeNEON_3 = (await activePool.getNEON()).toString();
    assert.equal(
      activeNEON_3.toString(),
      activeNEON_2.sub(toBN(_950_ZKUSD).mul(mv._1e18BN).div(price))
    );
  });
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint ZKUSD to Bob
    await zkusdToken.unprotectedMint(bob, dec(100, 18));

    assert.equal(await zkusdToken.balanceOf(bob), dec(100, 18));
    ...
    // Bob tries to redeem his illegally obtained ZKUSD
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint ZKUSD to Bob
    await zkusdToken.unprotectedMint(bob, "101000000000000000000");

    assert.equal(await zkusdToken.balanceOf(bob), "101000000000000000000");
    ...
    th.assertIsApproximatelyEqual(
      (await activePool.getZKUSDDebt()).toString(),
      totalDebt
    );
    ...
    // Bob attempts to redeem his ill-gotten 101 ZKUSD, from a system that has 100 ZKUSD outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    const A_balanceBefore = await zkusdToken.balanceOf(A);

    await th.redeemCollateral(A, contracts, dec(10, 18), GAS_PRICE);

    // Check A's balance has decreased by 10 ZKUSD
    assert.equal(
      await zkusdToken.balanceOf(A),
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await zkusdToken.balanceOf(A);
    const B_balanceBefore = await zkusdToken.balanceOf(B);
    ...
    // Check B's balance has decreased by 10 ZKUSD
    assert.equal(
      await zkusdToken.balanceOf(B),
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 ZKUSD
    assert.equal(
      A_balanceBefore.sub(await zkusdToken.balanceOf(A)),
      dec(10, 18)
    );
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero NEONFee to ZKT staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 ZKT
    await th.fastForwardTime(
      timeValues.SECONDS_IN_ONE_YEAR,
      web3.currentProvider
    ...
    // Check ZKT Staking contract balance before is zero
    const zktStakingBalance_Before = await web3.eth.getBalance(
      zktStaking.address
    );
    assert.equal(zktStakingBalance_Before, "0");
    ...
    // Check ZKT Staking contract balance after is non-zero
    const zktStakingBalance_After = toBN(
      await web3.eth.getBalance(zktStaking.address)
    );
    assert.isTrue(zktStakingBalance_After.gt(toBN("0")));
  });

  it("redeemCollateral(): a redemption made at zero base increases the NEON-fees-per-ZKT-staked in ZKT Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 ZKT
    await th.fastForwardTime(
      timeValues.SECONDS_IN_ONE_YEAR,
      web3.currentProvider
    ...
    // Check ZKT Staking NEON-fees-per-ZKT-staked before is zero
    ...
    // Check ZKT Staking NEON-fees-per-ZKT-staked after is non-zero
    const F_NEON_After = await zktStaking.F_NEON();
    assert.isTrue(F_NEON_After.gt("0"));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero NEONFee to ZKT staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 ZKT
    await th.fastForwardTime(
      timeValues.SECONDS_IN_ONE_YEAR,
      web3.currentProvider
    ...
    const zktStakingBalance_Before = toBN(
      await web3.eth.getBalance(zktStaking.address)
    );
    ...
    // check ZKT Staking balance has increased
    assert.isTrue(zktStakingBalance_After.gt(zktStakingBalance_Before));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate increases NEON-per-ZKT-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 ZKT
    await th.fastForwardTime(
      timeValues.SECONDS_IN_ONE_YEAR,
      web3.currentProvider
    ...
    // check ZKT Staking balance has increased
    assert.isTrue(F_NEON_After.gt(F_NEON_Before));
  });

  it("redeemCollateral(): a redemption sends the NEON remainder (NEONDrawn - NEONFee) to the redeemer", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 ZKT
    await th.fastForwardTime(
      timeValues.SECONDS_IN_ONE_YEAR,
      web3.currentProvider
    ...
    // Check total ZKUSD supply
    const activeZKUSD = await activePool.getZKUSDDebt();
    const defaultZKUSD = await defaultPool.getZKUSDDebt();

    const totalZKUSDSupply = activeZKUSD.add(defaultZKUSD);
    th.assertIsApproximatelyEqual(totalZKUSDSupply, totalDebt);
    ...
    /*
    At NEON:USD price of 200:
    NEONDrawn = (9 / 200) = 0.045 NEON
    NEONfee = (0.005 + (1/2) *( 9/260)) * NEONDrawn = 0.00100384615385 NEON
    NEONRemainder = 0.045 - 0.001003... = 0.0439961538462
    */

    const A_balanceAfter = toBN(await web3.eth.getBalance(A));

    // check A's NEON balance has increased by 0.045 NEON
    const price = await priceFeed.getPrice();
    const NEONDrawn = redemptionAmount.mul(mv._1e18BN).div(price);
    th.assertIsApproximatelyEqual(
      ...
      NEONDrawn.sub(
        toBN(dec(5, 15))
          ...
          .mul(NEONDrawn)
          .div(mv._1e18BN)
      ).sub(toBN(gasUsed * GAS_PRICE)), // substract gas used for troveManager.redeemCollateral from expected received NEON
      100000
    );
  });
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    // whale redeems 360 ZKUSD.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    // whale redeems ZKUSD.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, GAS_PRICE);
    ...
    /*
    At NEON:USD price of 200, with full redemptions from A, B, C:

    NEONDrawn from A = 100/200 = 0.5 NEON --> Surplus = (1-0.5) = 0.5
    NEONDrawn from B = 120/200 = 0.6 NEON --> Surplus = (1-0.6) = 0.4
    NEONDrawn from C = 130/200 = 0.65 NEON --> Surplus = (2-0.65) = 1.35
    */
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    // whale redeems ZKUSD.  Expect this to fully redeem A, B, C, and partially redeem 15 ZKUSD from D.
    ...
    /* Expect D to have lost 15 debt and (at NEON price of 200) 15/200 = 0.075 NEON.
    So, expect remaining debt = (85 - 15) = 70, and remaining NEON = 1 - 15/200 = 0.925 remaining. */
    const price = await priceFeed.getPrice();
    th.assertIsApproximatelyEqual(
      D_emittedDebt,
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's NEON surplus (collateral - NEON drawn) available for the trove owner to claim", async () => {
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's NEON surplus (collateral - NEON drawn) available for the trove owner after re-opening trove", async () => {
    const {
      A_netDebt,
      A_coll: A_collBefore,
      B_netDebt,
  ...
  it("redeemCollateral(): reverts if fee eats up all returned collateral", async () => {
    // --- SETUP ---
    const { zkusdAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraZKUSDAmount: dec(1, 24),
      extraParams: { from: alice },
    });
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const { firstRedemptionHint, partialRedemptionHintNICR } =
        await hintHelpers.getRedemptionHints(zkusdAmount, price, 0);
      ...
      await borrowerOperations.adjustTrove(
        ...
        alice,
        alice,
        { from: alice, value: zkusdAmount.mul(mv._1e18BN).div(price) }
      );
    }
  ...
  it("getPendingZKUSDDebtReward(): Returns 0 if there is no pending ZKUSDDebt reward", async () => {
    // Make some troves
    ...
    const current_L_ZKUSDDebt = await troveManager.L_ZKUSDDebt();
    assert.equal(current_L_ZKUSDDebt, 0);

    const carolSnapshot_L_ZKUSDDebt = (
      await troveManager.rewardSnapshots(carol)
    )[1];
    assert.equal(carolSnapshot_L_ZKUSDDebt, 0);

    const carol_PendingZKUSDDebtReward =
      await troveManager.getPendingZKUSDDebtReward(carol);
    assert.equal(carol_PendingZKUSDDebtReward, 0);
  });

  it("getPendingNEONReward(): Returns 0 if there is no pending NEON reward", async () => {
    // make some troves
    const { totalDebt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraZKUSDAmount: dec(100, 18),
    ...
    // Confirm there are no pending rewards from liquidation
    const current_L_NEON = await troveManager.L_NEON();
    assert.equal(current_L_NEON, 0);

    const carolSnapshot_L_NEON = (await troveManager.rewardSnapshots(carol))[0];
    assert.equal(carolSnapshot_L_NEON, 0);

    const carol_PendingNEONReward = await troveManager.getPendingNEONReward(
      carol
    );
    assert.equal(carol_PendingNEONReward, 0);
  });
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    // to be able to repay:
    await zkusdToken.transfer(B, B_totalDebt, { from: A });
  ...
  // skip("redeemCollateral(): owner can redeem their entire ZKUSDToken balance", async () => {
  //   const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
  //     ICR: toBN(dec(3, 18)),
  //     extraParams: { from: alice },
  //   });
  ...
  //   );
  //
  //   console.log((await zkusdToken.balanceOf(alice)).toString())
  //   // Erin attempts to redeem 400 ZKUSD
  //   const { firstRedemptionHint, partialRedemptionHintNICR } =
  //     await hintHelpers.getRedemptionHints(dec(1, 18), price, 0);
  //
  ...
  //     alice,
  //     alice,
  //     ethers.BigNumber.from('5500000000000000000'),
  //     0,
  //     th._100pct,
  //     { from: alice }
  //   );
