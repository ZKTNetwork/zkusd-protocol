...
contract(
  ...
  async (accounts) => {
    ...
    it("Sets the correct ZKUSDToken address in TroveManager", async () => {
      const zkusdTokenAddress = zkusdToken.address;

      const recordedClvTokenAddress = await troveManager.zkusdToken();

      assert.equal(zkusdTokenAddress, recordedClvTokenAddress);
    });
    ...
    // ZKT Staking in TroveM
    it("Sets the correct ZKTStaking address in TroveManager", async () => {
      const zktStakingAddress = zktStaking.address;

      const recordedZKTStakingAddress = await troveManager.zktStaking();
      assert.equal(zktStakingAddress, recordedZKTStakingAddress);
    });
    ...
    it("Sets the correct ZKUSDToken address in StabilityPool", async () => {
      const zkusdTokenAddress = zkusdToken.address;

      const recordedClvTokenAddress = await stabilityPool.zkusdToken();
    ...
    // ZKT Staking in BO
    it("Sets the correct ZKTStaking address in BorrowerOperations", async () => {
      const zktStakingAddress = zktStaking.address;

      const recordedZKTStakingAddress =
        await borrowerOperations.zktStakingAddress();
      assert.equal(zktStakingAddress, recordedZKTStakingAddress);
    });

    // --- ZKT Staking ---

    // Sets ZKToken in ZKTStaking
    ...
    // Sets ActivePool in ZKTStaking
    it("Sets the correct ActivePool address in ZKTStaking", async () => {
      const activePoolAddress = activePool.address;

      const recordedActivePoolAddress = await zktStaking.activePoolAddress();
      assert.equal(activePoolAddress, recordedActivePoolAddress);
    });

    // Sets ZKUSDToken in ZKTStaking
    it("Sets the correct ActivePool address in ZKTStaking", async () => {
      const zkusdTokenAddress = zkusdToken.address;

      const recordedZKUSDTokenAddress = await zktStaking.zkusdToken();
      assert.equal(zkusdTokenAddress, recordedZKUSDTokenAddress);
    });

    // Sets TroveManager in ZKTStaking
    it("Sets the correct ActivePool address in ZKTStaking", async () => {
      const troveManagerAddress = troveManager.address;

      const recordedTroveManagerAddress =
        await zktStaking.troveManagerAddress();
      assert.equal(troveManagerAddress, recordedTroveManagerAddress);
    });

    // Sets BorrowerOperations in ZKTStaking
    it("Sets the correct BorrowerOperations address in ZKTStaking", async () => {
      const borrowerOperationsAddress = borrowerOperations.address;

      const recordedBorrowerOperationsAddress =
        await zktStaking.borrowerOperationsAddress();
      assert.equal(
        borrowerOperationsAddress,
        recordedBorrowerOperationsAddress
      );
    ...
    // Sets CI in ZKToken
    it("Sets the correct CommunityIssuance address in ZKToken", async () => {
      const communityIssuanceAddress = communityIssuance.address;

      const recordedcommunityIssuanceAddress =
        await zkToken.communityIssuanceAddress();
      assert.equal(communityIssuanceAddress, recordedcommunityIssuanceAddress);
    });

    // Sets ZKTStaking in ZKToken
    it("Sets the correct ZKTStaking address in ZKToken", async () => {
      const zktStakingAddress = zktStaking.address;

      const recordedZKTStakingAddress = await zkToken.zktStakingAddress();
      assert.equal(zktStakingAddress, recordedZKTStakingAddress);
    });

    // Sets LCF in ZKToken
    it("Sets the correct LockupContractFactory address in ZKToken", async () => {
      const LCFAddress = lockupContractFactory.address;

      const recordedLCFAddress = await zkToken.lockupContractFactory();
      assert.equal(LCFAddress, recordedLCFAddress);
    });

    // --- LCF  ---

    // Sets ZKToken in LockupContractFactory
    ...
    // Sets ZKToken in CommunityIssuance
