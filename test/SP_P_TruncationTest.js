...
contract("StabilityPool Scale Factor issue tests", async (accounts) => {
  ...
  const getZKUSDAmountForDesiredDebt = async (desiredDebt) =>
    (await getOpenTroveZKUSDAmount(dec(desiredDebt, 18))).add(th.toBN(1));
  ...
  describe("Scale Factor issue tests", async () => {
    ...
    it("1. Liquidation succeeds after P reduced to 1", async () => {
      ...
      await zkusdToken.transfer(A, dec(50000, 18), { from: whale });

      // Open 3 Troves with 2000 ZKUSD debt
      for (account of [A, B, C]) {
        await borrowerOperations.openTrove(
          th._100pct,
          await getZKUSDAmountForDesiredDebt(2000),
          account,
          account,
    ...
    it("2. New deposits can be made after P reduced to 1", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await zkusdToken.transfer(newDepositors[i], newDeposits[i], {
          from: whale,
        });
        ...
        assert.isTrue(
          (await stabilityPool.getCompoundedZKUSDDeposit(newDepositors[i])).eq(
            newDeposits[i]
          )
        );
      }
    ...
    it("3. Liquidation succeeds when P == 1 and liquidation has newProductFactor == 1e9", async () => {
      ...
      const deposit_2 = deposit_0
        .sub(await stabilityPool.getTotalZKUSDDeposits())
    ...
    it("5. Depositor have correct depleted stake after deposit at P == 1 and scale changing liq (with newProductFactor == 1e9)", async () => {
      ...
      // D makes deposit of 1000 ZKUSD
      const D_deposit = dec(1, 21);
      await zkusdToken.transfer(D, dec(1, 21), { from: whale });
      ...
      const D_depletedDeposit = await stabilityPool.getCompoundedZKUSDDeposit(
        D
      );
