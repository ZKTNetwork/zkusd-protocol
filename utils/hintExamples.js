...
async function main() {
  ...
  const ZKTContracts = await dh.deployZKTContractsHardhat(
      ARBITRARY_ADDRESS, 
      ARBITRARY_ADDRESS,
      ARBITRARY_ADDRESS
    )
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  await dh.connectCoreContracts(coreContracts, ZKTContracts)
  await dh.connectZKTContracts(ZKTContracts)
  await dh.connectZKTContractsToCore(ZKTContracts, coreContracts)
  ...
  // Total debt of the new trove = ZKUSD amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = ZKUSDAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, ZKUSDAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const ZKUSDRepayment = toBN(toWei('230')) // borrower wants to repay 230 ZKUSD

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)
  
  const newDebt = debt.sub(ZKUSDRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, ZKUSDRepayment, false, upperHint, lowerHint, {value: collIncrease})
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated ZKUSD amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncatedZKUSDAmount,
    firstRedemptionHint,
