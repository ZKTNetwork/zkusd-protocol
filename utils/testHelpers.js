...
class TestHelper {
    ...
    static async openTrove_allAccounts(accounts, contracts, ETHAmount, ZKUSDAmount) {
        ...
        for (const account of accounts) {
            ...
            const tx = await contracts.borrowerOperations.openTrove(this._100pct, ZKUSDAmount, upperHint, lowerHint, {
                from: account,
                value: ETHAmount
            })
    ...
    static async openTrove_allAccounts_randomETH_ProportionalZKUSD(minETH, maxETH, accounts, contracts, proportion) {
        ...
        for (const account of accounts) {
            ...
            const tx = await contracts.borrowerOperations.openTrove(this._100pct, proportionalZKUSD, upperHint, lowerHint, {
                from: account,
                value: randCollAmount
            })
            const gas = this.gasUsed(tx)
    ...
    static async openTrove_allAccounts_randomZKUSD(minZKUSD, maxZKUSD, accounts, contracts, ETHAmount) {
        ...
        for (const account of accounts) {
            ...
            const tx = await contracts.borrowerOperations.openTrove(this._100pct, randZKUSDAmount, upperHint, lowerHint, {
                from: account,
                value: ETHAmount
            })
            const gas = this.gasUsed(tx)
    ...
    static async openTrove_allAccounts_decreasingZKUSDAmounts(accounts, contracts, ETHAmount, maxZKUSDAmount) {
        ...
        for (const account of accounts) {
            ...
            const tx = await contracts.borrowerOperations.openTrove(this._100pct, ZKUSDAmountWei, upperHint, lowerHint, {
                from: account,
                value: ETHAmount
            })
            const gas = this.gasUsed(tx)
    ...
    static async adjustTrove_allAccounts(accounts, contracts, ETHAmount, ZKUSDAmount) {
        ...
        for (const account of accounts) {
            ...
            const {
                newColl,
                newDebt
            } = await this.getCollAndDebtFromAdjustment(contracts, account, ETHChangeBN, ZKUSDChangeBN)
            ...
            if (ETHChangeBN.gt(zero)) {
                tx = await contracts.borrowerOperations.adjustTrove(this._100pct, 0, ZKUSDChangeBN, isDebtIncrease, upperHint, lowerHint, {
                    from: account,
                    value: ETHChangeBN
                })
    ...
    static async redeemCollateralAndGetTxObject(redeemer, contracts, ZKUSDAmount, gasPrice, maxFee = this._100pct) {
        ...
        const price = await contracts.priceFeedTestnet.getPrice()
        return await this.performRedemptionTx(redeemer, price, contracts, ZKUSDAmount, maxFee, gasPrice)
    }
    ...
    static async performRedemptionTx(redeemer, price, contracts, ZKUSDAmount, maxFee = 0, gasPrice_toUse = 0) {
        ...
        return await contracts.troveManager.redeemCollateral.sendTransaction(ZKUSDAmount,
            firstRedemptionHint,
    ...
    static async getLCFromDeploymentTx(deployedLCTx) {
        const deployedLCAddress = this.getLCAddressFromDeploymentTx(deployedLCTx)  // grab addr of deployed.json contract from event
